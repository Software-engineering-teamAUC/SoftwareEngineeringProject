                ...
                database.collection("books")
                        ...
                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                Toast.makeText(Onclick_uploaded_requested_books.this, "Book request accepted!", Toast.LENGTH_SHORT).show();
                                for(int tok = 0; tok < tokens.size(); tok++)
                                {
                                    FCMSend.pushNotification(
                                            Onclick_uploaded_requested_books.this,
                                            tokens.get(tok),
                                            "Request Accepted",
                                            "Your request on the book: " + Book_key.split("_")[1] +  " has been accepted"
                                    );
                                }
                                    ...
                                    .addOnSuccessListener(aVoid2 -> {
                                        ...
                                        for(int tok = 0; tok < tokens.size(); tok++)
                                        {
                                            FCMSend.pushNotification(
                                                    ...
                                                    tokens.get(tok),
                                                    "Request Declined",
                                                    "Your request on the book: " + Book_key.split("_")[1] +  " has been declined"
                                            );
                                        }
                                        Intent refresh = new Intent(Onclick_uploaded_requested_books.this, uploaded_requested_books.class);
                                        refresh.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NO_ANIMATION);
                        ...
                        if (document2.exists())
                        {
                            ...
                            for(int tok = 0; tok < tokens.size(); tok++)
                            {
                                FCMSend.pushNotification(
                                        Onclick_uploaded_requested_books.this,
                                        tokens.get(tok),
                                        "Book marked as delivered!",
                                        "The owner of the book: " + Book_key.split("_")[1] +  " has marked it as delivered"
                                );
                            }
                        }
                    }
