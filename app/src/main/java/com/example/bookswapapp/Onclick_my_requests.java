...
//                    genre.setText(Book_Genres);
        ...
        Cancel.setOnClickListener(new View.OnClickListener() {
            ...
            {
                database.collection("users").document(Book_key.split("_")[0]).get().addOnCompleteListener(task2 -> {
                    if (task2.isSuccessful()) {
                        DocumentSnapshot document2 = task2.getResult();
                        if (document2.exists())
                        {
                                    ...
                                    .addOnSuccessListener(aVoid2 -> {
                                        ...
                                        for(int tok = 0; tok < tokens.size(); tok++)
                                        {
                                            FCMSend.pushNotification(
                                                    Onclick_my_requests.this,
                                                    tokens.get(tok),
                                                    "Request Cancelled",
                                                    "A request on your book: " + Book_key.split("_")[1] +  " has been cancelled"
                                            );
                                        }
                        ...
                        if (document2.exists())
                        {
                            ...
                            for(int tok = 0; tok < tokens.size(); tok++)
                            {
                                FCMSend.pushNotification(
                                        Onclick_my_requests.this,
                                        tokens.get(tok),
                                        "Book marked as received!",
                                        "Your book: " + Book_key.split("_")[1] +  " has been marked as received"
                                );
                            }
                        }
                    }
